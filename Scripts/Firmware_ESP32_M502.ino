...
// Implementacion propia sin ModbusIP_ESP8266 — usa FreeRTOS + WiFiServer
//
// Sesiones concurrentes : MAX_SESSIONS (configurable, default 8)
// Funcion codes soportados:
//   0x01 — Read Coils
...
//   COIL  0        → LED GPIO2
//   HREG  100-105  → Servos (Pinza, PinzaRot, Muneca, Codo, Hombro, Base)
//
...
#define LOCK_MAGIC        0xA5A5
#define LOCK_TTL_SECONDS  15    // segundos sin renovar → auto-release
#define LOCK_NONE         0xFF  // sin duenio

// ─── Servidor Modbus TCP ──────────────────────────────────────────────
#define MODBUS_PORT     502
#define MAX_SESSIONS    8     // igual que S7-1200 / M221
#define SESSION_TIMEOUT 30000 // ms sin actividad → cerrar sesion

// ─── Identificacion de fabricante (Fn 0x43) ───────────────────────────
...
bool     mbCoils[NUM_COILS]  = {false};
uint16_t mbHregs[NUM_HREGS]  = {0};

// ─── Estado del control exclusivo ────────────────────────────────────
// Protegido por xRegMutex
struct ExclusiveLock {
    bool     active       = false;
    uint8_t  ownerSession = LOCK_NONE;  // ID de sesion Modbus duenia
    uint16_t token        = 0;          // token aleatorio
    uint32_t expiresAt    = 0;          // millis() de expiracion
} exclusiveLock;

// ─── Servos y animacion ───────────────────────────────────────────────
Servo servos[NUM_SERVOS];
float currentAngles[NUM_SERVOS]    = {90,90,90,90,90,90};
int   lastTargetAngles[NUM_SERVOS] = {90,90,90,90,90,90};
const float VELOCIDAD_SERVO = 0.060f;
...
struct ModbusSession {
    ...
    uint8_t     id          = 0;
    // Buffers en el struct (memoria estatica) para no presionar el stack de la tarea
    uint8_t     rxBuf[512];
    uint8_t     txBuf[512];
    uint8_t     pduReq[256];
    uint8_t     pduResp[256];
...
ModbusSession sessions[MAX_SESSIONS];
...
// Leer holding register con mutex
...
// Escribir holding register con mutex
...
// =====================================================================
// GESTION DEL CONTROL EXCLUSIVO (Exclusive Control Lock)
// =====================================================================

// Actualizar registros de lectura del lock (llamar con mutex tomado)
void _syncLockRegs() {
...
// Retorna: true si se concede o ya lo tiene, false si lo tiene otra sesion.
bool lockAcquire(uint8_t sessionId) {
    ...
    if (!exclusiveLock.active) {
        ...
        exclusiveLock.ownerSession = sessionId;
        ...
        // Ya lo tiene → renovar TTL (heartbeat)
        exclusiveLock.expiresAt = millis() + (LOCK_TTL_SECONDS * 1000UL);
        _syncLockRegs();
        granted = true;
    } else {
        // Lo tiene otra sesion
...
// Liberar el lock (solo el duenio puede liberarlo)
...
// Forzar liberacion por timeout (llamado desde loop)
...
// (servos, LED). Retorna true si puede escribir.
bool lockCanWrite(uint8_t sessionId) {
    ...
    if (!exclusiveLock.active) {
        ...
        can = (exclusiveLock.ownerSession == sessionId);
    }
    xSemaphoreGive(xRegMutex);
    return can;
}
...
// =====================================================================
// PROCESADOR DE PDU MODBUS
// Recibe el PDU completo (sin MBAP), devuelve longitud de respuesta PDU
// sessionId: ID de la sesion que origina este PDU (para control de lock)
// =====================================================================
int processPDU(const uint8_t* req, int reqLen, uint8_t* resp, uint8_t sessionId) {
    if (reqLen < 1) return 0;
    uint8_t fc = req[0];

    // Macro auxiliar: determina si la direccion es de control de actuadores
    // (servos REG 100-105, LED coil 0). Estas direcciones respetan el lock.
    ...
    if (fc == 0x06) {
        ...
        // Registros de control de servos: respetar lock
    ...
    if (fc == 0x0F) {
        ...
        // Si incluye el coil LED, verificar lock
    ...
    if (fc == 0x10) {
        ...
        for (int i = 0; i < qty; i++) {
            ...
            // Registros de control: respetar lock
            if (IS_CTRL_HREG(addr) && !lockCanWrite(sessionId)) goto exception_lock;
        }
    ...
    if (fc == 0x2B) {
        if (reqLen < 2) goto exception_illegal;
        if (req[1] != 0x0E) goto exception_illegal;

        uint8_t readDevId = (reqLen >= 3) ? req[2] : 0x01;
    ...
    exception_lock:
    // Excepcion 0x08: Slave Device Busy — estandar para "recurso ocupado"
...
// =====================================================================
// TAREA DE SESION — se ejecuta en su propio core/stack de FreeRTOS
// Una tarea por cliente conectado, hasta MAX_SESSIONS
// =====================================================================
void sessionTask(void* param) {
    ...
    while (sess->active && sess->client.connected()) {
        ...
        // Sin este fix, se lee un byte de mas y el Function Code queda
        // desplazado, causando que processPDU no reconozca ningun FC.
        uint8_t  unitId  = rxBuf[6];  // Leer el Unit ID
        if (unitId != 0x01 && unitId != 0xFF) continue;
        ...
        Serial.printf("[DEBUG] pduLen=%d FC=0x%02X req[1]=0x%02X\n", pduLen, pduReq[0], pduReq[1]);
        int respLen = processPDU(pduReq, pduLen, pduResp, sess->id);
    ...
    // Liberar el lock si esta sesion lo tenia (simula desconexion abrupta)
    lockRelease(sess->id);
    sess->client.stop();
...
// =====================================================================
// TAREA DE ACEPTACION — escucha nuevas conexiones en puerto 502
// Se ejecuta en Core 0, separada del loop principal
// =====================================================================
void acceptTask(void* param) {
    ...
    while (true) {
        ...
        if (slot == -1) {
            // Sin slots disponibles — comportamiento identico a PLC real:
            // aceptamos la conexion TCP pero respondemos con excepcion
            // y cerramos (algunos PLCs directamente no responden)
        ...
        // Crear tarea FreeRTOS para esta sesion
        char taskName[16];
        snprintf(taskName, sizeof(taskName), "mb_sess_%d", slot);
        xTaskCreatePinnedToCore(
            sessionTask,
            taskName,
            8192,              // stack aumentado: buffers rxBuf+txBuf+pduReq+pduResp = ~1.5KB + overhead FreeRTOS + WiFiClient
            &sessions[slot],   // parametro
            5,                 // prioridad (por debajo de la tarea de aceptacion)
...
// =====================================================================
// DASHBOARD HTML (identico al original — solo se actualiza el titulo)
// =====================================================================
